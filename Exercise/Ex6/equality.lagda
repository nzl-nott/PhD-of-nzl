...
ordered pairs are belonging to |Id A|. We can describe it in
...
data Id' (A : Set)(a : A) : A → Set where
  refl : Id' A a a
\end{code}
This one is just what used in the Agda standard library. We can treat |Id A
a| it as a predicate of whether certain element of |A| is the same as
...
\end{code}
The P and m are both indexed by different 'a'. P is actually a ternary
relation.
...
J' A .b P m b refl = m
\end{code}
The P and m are now restricted by the same 'a' as the the identity
predicate. P and m here are actually special cases of the P (P [a])
and m (m [a]) in J. 'a' can be regarded as a constant in the
discourse.
 
J' actually maps  \begin{code}P a refl\end{code}
