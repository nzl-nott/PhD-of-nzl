...
\section{Set Theory v.s. Type Theory}
...
the Kleene-Rosser paradox\cite{kleene1935inconsistency}. Then he refined it with adding types. This theory is also called
...
\section{Per Martin-L\"{o}f's Type Theory}
...
\emph{``propositions can be interpreted as types and their
  proofs are inhabitants of these types''} 

Based on these, implications are functions, conjunctions are products, negations are functions into empty types, modus ponens is function application etc. Given any proposition (type), we can give a proof of it by construct a program of this type. There is no obvious distinction between computer science programs and proofs. This makes it a programming languages to write certified programs. Propositions are also very useful in the specification of programs. It is not necessary to verfify programs after finished it, restrictions can be carried along with the definition. The type systems of simply lambda calculus like Haskell are not expressive enough to encode predicate logic without dependent types.
...
\todo{Cosider moving this part to \autoref{HITs}}
...
\section{Extensional concepts in \itt}
