...
open import Relation.Binary.PropositionalEquality as PropEq
  using (_≡_; _≢_; refl; sym; cong; cong₂) renaming (trans to _>=<_)
...
private
  ...
  foldfz : ∀ n z → (f : ℕ → ℕ) → fold f (f z) n ≡ f (fold f z n)
  foldfz zero z f = refl
  foldfz (suc n) z f = cong f (foldfz n z f)

  foldcomm : ∀ m n z → (f : ℕ → ℕ) → fold f (fold f z n) m 
                                    ≡ fold f (fold f z m) n
  foldcomm m zero z f = refl
  foldcomm m (suc n) z f = foldfz m (fold f z n) f >=<
                           cong f (foldcomm m n z f)

  foldn : ∀ n → n ≡ fold suc zero n
  foldn zero = refl
  foldn (suc n) = cong suc (foldn n)
  ...
  m+1+n≡1+m+n m n = foldfz m n suc

  +-comm : Commutative _+_
  +-comm m n = {!>=< foldcomm m n zero suc!}

{-
zero = n+0≡n m
  +-comm m (suc n) = m+1+n≡1+m+n m n >=< cong suc (+-comm m n)
-}    
  ...
  *-zero = (λ _ → refl) , n*0≡0
  ...
  m*1+n=m*n+m : ∀ m n → m * (suc n) ≡ m * n + m
  m*1+n=m*n+m zero n = refl
  m*1+n=m*n+m (suc m) n = cong (λ x → x + suc n) (m*1+n=m*n+m m n) >=< {!m+1+n≡1+m+n !}
...
{-
zero = n*0≡0 m
  *-comm m (suc n) = m*1+n=m*n+m m n >=< cong (λ x → x + m) (*-comm m n)
-}
