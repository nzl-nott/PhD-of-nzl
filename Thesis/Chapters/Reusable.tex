and also $\Sigma$-types and quotient types are not discussed in Altenkirch's original construction. $\Sigma$-types are intuitively simple, but the construction of it in setoid models still requires a lot of unification proofs due to the type dependency. The more important one is the construction of quotient types.
...
module TyEq  (Γ : Con)(A B : Ty Γ) where

  ⟦TyEq⟧ : Ty Γ
  ⟦TyEq⟧ = record 
           { fm = λ γ → let A' =  ∣ [ A ]fm γ ∣ in
                        let B' =  ∣ [ B ]fm γ ∣ in 
             record { Carrier = Subset ((A' → B') × (B' → A')) 
                      (λ {(f , g) → 
                        (∀ x → f (g x) ≡ x) × (∀ y → g (f y) ≡ y)})
             ; _≈_ = λ x y → ∀ a → proj₁ (prj₁ x) a ≡ proj₁ (prj₁ y) a
             ; refl = λ a → PE.refl
             ; sym = λ x₁ a → PE.sym (x₁ a)
             ; trans = λ x₁ x₂ a → PE.trans (x₁ a) (x₂ a) }
           ; substT = λ p x → ((λ x₁ → [ B ]subst {!!} {!!}) , {!!}) , {!!}; subst* = {!!}; refl* = {!!}; trans* = {!!} }
