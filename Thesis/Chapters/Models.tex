...
It implies that $f$ does not depend computationally on this argument, hence $f(a) \equiv f(b)$ for any $a, b : A$. It can also used in dependent function types, dependent products (record types). 
...
\end{itemize}
 ...
 There is a small problem of irrelevant fields of record types as we will see later: we can not use a irrelevant value to construct a irrelevant field or irrelevant function. For example, we can not simply write $p$ in the place of $?$ in the following function
...
This metatheory is still decidable, consistent and should be adequate because the only axiom is irrelevant which can not be used to construct non-canonical terms of $\N$.
...
\begin{code}
\>[4] \AgdaField{subst*} \AgdaSymbol{=} \AgdaSymbol{λ} \AgdaBound{\_} \AgdaBound{x} \AgdaSymbol{→} \AgdaPostulate{irrelevant} \AgdaBound{x}\<%
\end{code}
...
we can build a quotient type if we have an equivalence relation $R$ defined on $A$ which have to respect the underlying equivalence of $A$.
