...
wkComm :  ∀{Γ Δ : Con}{A : Ty Γ}{B : Ty Δ}{γ}{C} → A [ γ ]T ≡ C → A [ γ +S B ]T ≡ C +T B
wkComm T = trans [+S]T (wk-T T)

1-1cm : ∀ {Γ Δ : Con}{A : Ty Γ}{B : Ty Δ} → 
        (γ : Γ ⇒ Δ) → B [ γ ]T ≡ A → (Γ , A) ⇒ (Δ , B)
1-1cm γ eq = (γ +S _) , (var v0 ⟦ wkComm eq ⟫)

1-1cm-same : ∀ {Γ : Con}{A B : Ty Γ} → 
            B ≡ A → (Γ , A) ⇒ (Γ , B)
1-1cm-same eq = (IdCm _ +S _) , (var v0 ⟦ trans [+S]T (wk-T (trans (IC-T _) eq)) ⟫)

1-1cm-same-T : ∀ {Γ : Con}(A B : Ty Γ) → 
               (eq : B ≡ A) → (A +T B) [ 1-1cm-same eq ]T ≡ A +T A
1-1cm-same-T A B eq = trans +T[,]T (trans [+S]T (wk-T (IC-T _)))


1-1cm-same-tm : ∀ {Γ : Con}{A B : Ty Γ} → 
               {eq : B ≡ A}(a : Tm A) → (a +tm B) [ 1-1cm-same eq ]tm ≅ (a +tm A)
1-1cm-same-tm a = +tm[,]tm a ∾ [+S]tm a ∾ cong+tm2 (IC-tm a)
...
Σv[Σs]v (v0 {Γ} {A}) (δ , a) = congtm (cohOpV (sym (ΣT[+T] A A))) ∾
                                 wk-coh ∾ cohOp (ΣT[Σs]T A δ) ∾ cohOpΣtm a +T[,]T -¹
Σv[Σs]v (vS {Γ} {A} {B} x) (δ , a) = congtm (cohOpV (sym (ΣT[+T] A B))) ∾
                                       +tm[,]tm (Σtm (var x)) ∾
...
Σtm[Σs]tm {Γ} {Δ} (JJ {Δ = Δ₁} x δ A) δ₁ = congtm (cohOp (sym (ΣT[Σs]T A δ))) ∾ cohOp (sym [⊚]T) ∾ JJ-eq (sym (Σs⊚ δ δ₁)) ∾ (cohOpΣtm (JJ x (δ ⊚ δ₁) A) (sym [⊚]T) ∾ cohOp (sym (ΣT[Σs]T A (δ ⊚ δ₁)))) -¹

Σs•-left-id : ∀{Γ Δ : Con}(γ : Γ ⇒ Δ) → Σs {Γ} • ≡ Σs {Δ} • ⊚ Σs γ
Σs•-left-id {ε} {ε} • = refl
Σs•-left-id {ε} {Δ , A} (γ , a) = trans (Σs•-left-id {ε} {Δ} γ) (sym (⊚wk (Σs {Δ} •)))
...
rep-T'-p1 : {Γ Δ : Con}(A : Ty Γ)(B C : Ty Δ) → rep-T' A (B +T C) ≡ rep-T' A B +T rep-T' A C
rep-T'-p1 A * C = refl
rep-T'-p1 A (a =h b) C = {!!} -- trans (wk-T {!!}) {!!}


rep-tm'-p1 : {Γ Δ : Con}(A : Ty Γ)(B : Ty Δ) → rep-tm' A B +T rep-T' A B ≡ rep-T' A (B +T B)
rep-tm'-p1 A * = refl
rep-tm'-p1 A (a =h b) = {!wk-T!}


rep-tm' A .(A₁ +T A₁) (var (v0 {Γ₁} {A₁})) = var v0 ⟦ rep-T'-p1 A A₁ A₁ ⟫
rep-tm' A .(A₁ +T B) (var (vS {Γ₁} {A₁} {B} x)) = (rep-tm' A A₁ (var x) +tm _) ⟦ rep-T'-p1 A A₁ B ⟫
rep-tm' A .(A₁ [ δ ]T) (JJ x δ A₁) = JJ {!!} (rep-cm' A δ) (rep-T' A A₁) ⟦ {!!} ⟫
...
{-
rep-T-p1-lemma : {Γ Θ : Con}{A : Ty Γ}(Δ : Ty Θ → Con){a b : Ty Θ}(eq : a ≡ b)(γ : Δ a ⇒ Γ) → A [ subst (λ x → Δ x ⇒ Γ) eq γ ]T ≡ subst (λ x → Ty (Δ x)) eq (A [ γ ]T)
rep-T-p1-lemma Δ refl γ = refl
-}
...
fci-l1 * = refl
fci-l1 (_=h_ {A} a b) = trans [⊚]T (trans 
                              (congT (trans (ΣT[Σs]T (ΣT (rep-T A *)) (rep-C-cm-spl2 A)) (cong ΣT (fci-l1 A)))) 
                              (hom≡ (congtm (cohOpV (sym (ΣT[+T] (rep-T A * +T rep-T A *) (rep-T A * +T rep-T A *)))
                                        ∾ cohOpVs (sym (ΣT[+T] (rep-T A *) (rep-T A *))) )
                                       ∾ wk-coh ∾ wk-coh ∾ wk-coh+ ∾ cong+tm2 (wk-coh+ ∾ cohOp (wk-T (IC-T (ΣT (rep-T A *)))))) 
                                    (congtm
                                       (cohOpV
                                        (sym (ΣT[+T] (rep-T A * +T rep-T A *) (rep-T A * +T rep-T A *))))
                                       ∾ wk-coh ∾ cohOp
                                                    (trans [+S]T
                                                     (wk-T
                                                      (trans (IC-T (ΣT (rep-T A * +T rep-T A *)))
                                                       (ΣT[+T] (rep-T A *) (rep-T A *))))))))
...
rpl-T Γ Δ B A = rep-T B A [ cor _ ]T

rpl-tm Γ Δ B a = rep-tm B a [ cor _ ]tm



rpl-split : {Γ : Con}(Δ : Con)(A : Ty Γ)(B : Ty Δ) → (rpl Γ Δ A , rpl-T Γ Δ A B) ⇒ rpl Γ (Δ , B) A
rpl-split Δ A B = id-cm++ _ _ (rep-C-cm-spl A B)

rpl' : (Γ Δ : Con) → (A : Ty Γ) → Con


rpl'-p1 : {Γ : Con}(Δ : Con)(A : Ty Γ) → rpl' Γ Δ A ⇒ rpl Γ Δ A

rpl' Γ ε A = Γ
rpl' Γ (Δ , B) A = rpl' Γ Δ A , rpl-T Γ Δ A B [ rpl'-p1 Δ A ]T -- rep-C' A Δ , {!!} -- rep-T A B [ rep-C'-p1 A Δ ]T


rpl'-p1 ε A = IdCm _ ++cm filter-cm A
rpl'-p1 (Δ , B) A = rpl-split _ A B ⊚ ((rpl'-p1 Δ A +S _) , var v0 ⟦ [+S]T ⟫) 
