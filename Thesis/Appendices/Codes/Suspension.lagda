...
Σtm[Σs]tm {Γ} {Δ} (coh {Δ = Δ₁} x δ A) δ₁ = congtm (cohOp 
                       (sym (ΣT[Σs]T A δ))) 
                      ∾ cohOp (sym [⊚]T) 
                      ∾ coh-eq (sym (Σs⊚ δ δ₁)) 
...
Σs•-left-id {Γ , A} {ε} • = trans (cong (λ x → x +S ΣT A)
  (Σs•-left-id {Γ} {ε} •)) (S-eq (S-eq refl ([+S]V (vS v0) {Σs• Γ} -¹))
  ([+S]V v0 {Σs• Γ} -¹))
Σs•-left-id {Γ , A} {Δ , A₁} (γ , a) = trans (Σs•-left-id γ) 
  (sym (⊚wk (Σs• Δ))) 

Σs⊚ • γ = Σs•-left-id γ
Σs⊚ {Δ} (_,_ δ {A} a) γ = S-eq (Σs⊚ δ γ) (cohOp (ΣT[Σs]T A (δ ⊚ γ)) 
  ∾ cohOpΣtm (a [ γ ]tm) [⊚]T ∾ (cohOp [⊚]T ∾ congtm 
  (cohOp (ΣT[Σs]T A δ)) ∾ Σtm[Σs]tm a γ) -¹) 
...
filter (Δ , A) A₁ =  ΣC-it-S-spl A₁ A ⊚ ((filter Δ A₁ +S _) , 
  var v0 ⟦ [+S]T ⟫)

rpl-T-p1 : {Γ : Con}(Δ : Con)(A : Ty Γ) → rpl-T A * ≡ A [ rpl-pr1 Δ A ]T
rpl-T-p1 ε A = trans (ΣT-it-p1 A) (sym IC-T)
rpl-T-p1 (Δ , A) A₁ = trans [⊚]T (trans (congT (ΣT-it-wk A₁ A)) 
  (trans +T[,]T (trans [+S]T (trans (wk-T (rpl-T-p1 Δ A₁)) 
  (sym [+S]T)))))
