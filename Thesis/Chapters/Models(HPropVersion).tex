...
In this chapter, we mainly introduce an implementation of Altenkirch's setoid model  \cite{alti:lics99}  where types are interpreted as setoids. We define the model as categories with families in Agda with h-propositions instead of postulating $\Prop$ universe. We introduce some basic types in the setoid model. It has been shown by Altenkirch \cite{alti:lics99} that functional extensionality is inhabited in this model. More importantly, because types are interpreted as setoids, quotient types can be defined simply by replacing equality in a given setoid.
...
\begin{remark}[The category of setoids is not LCCC]
...
families but not every category with families has to be a LCCC. 
 Altenkirch and Klaus have shown that both \textbf{Gpd} and \textbf{Std} are Cartesian closed but not locally Cartesian closed in  \cite{Altenkirch12setoidsare}.
\end{remark}
...
For readability, we will omit some unnecessary code. The complete code can be found in \autoref{app:cwf}.
...
To complete the metatheory, we first define the h-propositions used in \hott to replace the axiomatic proof-irrelevant universe of propositions. An $\HProp$ (or \textbf{hProp}) consists a set $\AgdaField{prf}$ and a property that its identity proof is unique. Given an $A : \HProp$ We also use syntactic notation $< A >$ to project out the underlying proof term.
...
It is not exactly the same as $\Prop$. Because we can claim a set is in $\Prop$ and it has proof-irrelevance property, but we can not prove a set is in \textbf{Prop}. However any set which behaves like a proposition is an \textbf{HProp} . Another difference is that the uniqueness of identity proof is a propositional equality instead of definitional equality which complicates the proofs.

We also define basic propositions $\top$ and $\bot$ simply by unit type and empty type because they behaves like propositions:
...
To define the universal and existential quantifier, namely $\HProp$ is closed under $\Pi$-types and $\Sigma$-types, there is a problem proving $\Pi$-closure. It requires functional extensionality to prove the uniqueness which should be considered as proof-irrelevant, although in principle, we lose canonicity.
...
Notice that we use a syntactic abbreviation $<\_>$ for projection of proof in $\textbf{HProp}$.  The symbol $'$ is used to avoid conflicts with existing notations.

To prove it is closed under $\Sigma$-types, we use an auxiliary function $\mathsf{sig-eq}$ to combine 
equalities for both parts of a $\Sigma$-type.
...
The $\eta$-rules for both $\Pi$-types and $\Sigma$-types automatically holds in Agda.
...
To summarise, this metatheory is still decidable, consistent but not adequate because of the axiom of functional extensionality.

\subsection{Category of setoids}

Because we have $\HProp$ in our metatheory, we modified the definition of setoids by turning the equivalence relation into a propositional one.
...
We observe that \textbf{HProp} is not considered as a sort of types, so we simplified the definition by defining the laws non-propositionally, e.g.\ reflexivity.
...
The substitution of terms is similar so that we omit it here.


To complete the categories with families, the terminal object is just the empty context
\begin{code}
%
\\
...
Q-\textbf{Ax} can be simply proved because the underlying equivalence is definitionally equal to the equvalence relation $R$.
