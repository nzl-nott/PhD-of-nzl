...
\infrule[$\J$]{\Gamma , x : A, y : A, p : x =_A y \vdash C(x,y,p)  \andalso \Gamma , x : A \vdash t(a) : C(x,x,r(x)) \\ \andalso \Gamma \vdash a: A \andalso \Gamma \vdash a': A \andalso \Gamma \vdash p : a =_A a'}{\Gamma \vdash \J (t,a,a',p): C(a',a',p)}
...
\infrule[$\top$-elim]{\Gamma ,x : \top \vdash A \andalso \Gamma \vdash t : A[*/x] }{ \vdash t : A}
...
As we mentioned before, quotient types (see \autoref{iqs}) are available in \hott. Because of the different interpretations of types, it makes less confusion to call them \emph{quotients} here.
