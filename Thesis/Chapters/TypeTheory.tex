...
$$\text{subst}(B,p) \defeq : \J (t,a,a',p) : B(a) \to B(a')$$

For simplicity, if we have a term of $b : B(a)$, we write $\text{subst}(B,p,b) : B(a')$ as the result.
\end{definition}
...
Functional extensionality is derivable from equality reflection rule \ref{reflection}
\end{theorem}
\begin{proof}
...
It is easy to see that $0$ is an element for $B(\text{id})$. By applying subst function (see \autoref{subst}), we can construct an element of $B(\text{id}')$ as

$$\text{subst}(B,(\text{ext}(p),0)  : B(\text{id}')$$
